...
音声やDiscordのボイスチャットで聞こえてくる外国語をリアルタイムで文字起こし・翻訳するWindowsデスクトップアプリです。ゲームプレイ中の音声翻訳はもちろん、画面上の任意の範囲を選択してテキストを翻訳する「OCR単発翻訳」機能により、海外ニュース記事や画像内の文章など、日常的な用途にも幅広く使えます。
...
- Ollama使用時、Wikipedia記事の抜粋などの「参考コンテキスト」を貼り付けておくと、認識開始時に固有名詞の対訳用語集が自動抽出され、以降の翻訳に反映される
  - 自動抽出に頼らず、「手動用語集」欄に`原文 => 訳語`形式で直接指定することも可能(同じ原文がある場合は手動側が優先)
...
- グローバルホットキー対応(既定はCtrl+Alt+R: 開始/停止、Ctrl+Alt+O: オーバーレイ切り替え、Ctrl+Alt+T: OCR単発翻訳)。ゲームがアクティブな状態でもAlt-Tabせず操作できる。組み合わせは設定画面の「ショートカット」タブから変更可能
- **OCR単発翻訳**: 画面上の好きな範囲をドラッグ選択し、写っているテキスト(ニュース記事・ゲーム内チャット欄・画像内の文字など)をWindows標準OCRで読み取って翻訳するモード。VC翻訳の実行有無に関わらず単独で使え、ショートカット(既定Ctrl+Alt+T)または訳文オーバーレイの🔍ボタンから起動できる
  - 認識対象言語を設定画面の「言語」タブから選択可能(英語/日本語/韓国語/中国語/スペイン語/フランス語/ドイツ語/ロシア語/ポルトガル語)。対象言語のWindows言語パックが必要
  - 認識結果と訳文を非モーダルなポップアップで表示。「訳文をコピー」でクリップボードにコピー可能
- `.env`ファイルによる設定管理(数値はカルチャに依存しない形式で保存、保存はアトミック(tmp書き込み→置換)で行いクラッシュ時の破損を防止)
- ゲームプロファイル機能。翻訳バックエンド・翻訳先言語・参考コンテキスト・手動用語集・オーバーレイ表示設定を、ゲームごとに名前を付けて保存・切り替え可能(`game_profiles.json`)
- 診断ログ(`logs/app-YYYYMMDD.log`)。翻訳API呼び出しの失敗など、握りつぶさず記録が必要な例外を出力する
- 処理が追いつかず音声セグメントが破棄された場合、件数をステータス欄に表示

## 動作環境

- Windows 10/11(OCR単発翻訳機能を使う場合はWindows 10 1809以降が必要)
...
# Ollama使用時、ユーザーが直接入力する手動用語集("original => translation"形式、1行1エントリ。改行は\nでエスケープ済み)
MANUAL_GLOSSARY=
...
OCR_HOTKEY_MODIFIERS=Control, Alt
OCR_HOTKEY_KEY=T

# OCR単発翻訳で認識するテキストの言語(BCP-47に近い簡易コード。例: en, ja, ko, zh)
# 設定画面の「言語」タブからも選択可能
OCR_SOURCE_LANGUAGE_TAG=en
```
...
OCR単発翻訳機能([`Windows.Media.Ocr`](https://learn.microsoft.com/uwp/api/windows.media.ocr))を使えるようにするため、プロジェクトのターゲットフレームワークは`net8.0-windows10.0.19041.0`(Windows SDK対応版)を指定しています。追加のNuGetパッケージやVisual Studioのワークロードのインストールは不要ですが、初回`dotnet restore`時にWindows SDK向けのターゲッティングパックがNuGetから取得されるため、インターネット接続が必要です。
...
1. アプリを起動し、「設定」ボタンから各種設定を行って保存する
   ...
   - **言語**: 認識のヒント・認識言語(原文)・翻訳先言語(訳文)・(Ollama使用時)参考コンテキスト・OCR認識言語
   - **オーバーレイ**: 文字サイズ・背景の不透明度・最大表示行数
   - **ショートカット**: グローバルホットキー(開始/停止・オーバーレイ切り替え・OCR単発翻訳)の割り当て変更
...
8. OCR単発翻訳: ショートカット(既定Ctrl+Alt+T)、またはオーバーレイ表示中はその🔍ボタンから起動。画面が半透明になるので翻訳したい範囲をドラッグ選択(Escで取消)すると、認識結果と訳文がポップアップで表示される
...
自動抽出はAIによる推測が入るため、必ずしも意図した訳語になるとは限りません。特定の固有名詞の訳語を確実に固定したい場合は、同じ「翻訳」タブの「手動用語集」欄に、`原文 => 訳語`の形式(1行1エントリ)で直接指定できます。参考コンテキストが未入力でも手動用語集だけで機能し、同じ原文が両方に存在する場合は手動用語集側が優先されます。

## OCR単発翻訳機能について

画面上の任意の範囲を選択して、そこに写っているテキストを翻訳する機能です。VC翻訳やゲームを前提とせず、海外のニュース記事・PDF・画像内の文章など、日常的な用途にも使えるよう設計しています。

- 起動方法は2通りあります: グローバルホットキー(既定Ctrl+Alt+T)、または訳文オーバーレイを表示している場合はその🔍ボタン
- 起動すると画面全体が半透明になるので、翻訳したい範囲をドラッグで選択します(Escでキャンセル)
- 選択範囲はスクリーンショットとして取得され、Windows標準のOCRエンジン([`Windows.Media.Ocr`](https://learn.microsoft.com/uwp/api/windows.media.ocr))でテキストを抽出したうえで、設定済みの翻訳バックエンド(DeepL/Ollama、キャッシュ・フォールバックも含めVC翻訳と共通)で翻訳します
- 結果は認識したテキストと訳文を並べた非モーダルなポップアップで表示され、「訳文をコピー」でクリップボードにコピーできます
- 認識対象の言語は設定画面の「言語」タブの「OCR認識言語」で選択します。**対象言語のWindows言語パックが端末にインストールされている必要があります**(未導入の場合、実行時にエラーメッセージで案内されます。Windowsの設定→時刻と言語→言語 から追加できます)
- VC翻訳の実行中/停止中を問わず、いつでも独立して使えます

## ゲームプロファイル機能について

設定画面の「ゲームプロファイル」タブから、「翻訳エンジン」「言語」「オーバーレイ」タブの内容(翻訳バックエンド・翻訳先言語・参考コンテキスト・手動用語集・オーバーレイの文字サイズ/不透明度/最大表示件数/文字色)をゲームごとに名前を付けて保存し、後から切り替えられます。

- デバイス選択・ホットキー・DeepL/OllamaのAPIキーやエンドポイントなどは対象外です(プロファイルを切り替えても変わりません)
- プロファイルは`.env`とは別に、実行ファイルと同じフォルダの`game_profiles.json`に保存されます
- 「読み込む」ボタンを押した時点ではこの設定画面上の入力欄が置き換わるだけで、実際にアプリへ反映されるのは、設定画面自体を「保存」した時です
- 「保存」ボタン(プロファイル側)は、設定画面自体を保存していなくても、その時点で画面に入力中の内容をそのまま新しいプロファイルとして保存します
...
- [`Windows.Media.Ocr`](https://learn.microsoft.com/uwp/api/windows.media.ocr) — OCR単発翻訳のテキスト抽出(Windows標準搭載、追加パッケージ不要)
...
- OCR単発翻訳は、画像内のロゴ・アイコン・装飾的な記号を意味のない文字として誤認識することがある(例: 著作権マークを1文字のアルファベットとして認識するなど)。本文だけを狭めに範囲選択すると誤認識を減らせる
- DeepL使用時、APIキーの誤り・権限エラー(401/403)はOllamaへ自動フォールバックしない(設定ミスに気づけなくなるため、意図的にエラーとして表示する)。一方、利用上限超過(429/456)はOllamaへフォールバックしつつ、訳文と併せて「利用上限に達しています」という警告も表示する
...
- **診断ログ**(`logs/`フォルダ、exeと同じ場所に作成)には、例外発生時のスタックトレースや、DeepL/OllamaのHTTPエラーレスポンス本文を記録します。エラーレスポンス本文には、リクエストした文章(＝実際の発話内容、OCR単発翻訳の場合は画面から認識したテキスト)の断片がそのままエコーバックされて含まれる場合があります。ログは起動のたびに30日より古いものを自動削除し、1日あたりの計測ログ(`metrics-*.log`)は50MBで書き込みを打ち切りますが、無期限に保存され続けるものではないという前提で、共有や配布の際は`logs/`フォルダの中身を確認・除外することを推奨します(`.gitignore`でも`logs/`はGit管理対象外にしています)。
- **OCR単発翻訳のスクリーンショット**は、選択範囲をメモリ上でOCR処理するためだけに使用し、ディスクには一切保存しません(処理後は破棄されます)。認識したテキストと訳文は、他の翻訳結果と同様にメモリ上にのみ保持され、アプリを終了すると消えます。
...
- **ゲームプロファイル**(`game_profiles.json`、exeと同じ場所に作成)には、参考コンテキスト・手動用語集など、設定画面に入力した内容がプレーンテキストのまま保存されます(APIキーやデバイス情報は含まれません)。共有・配布の際は内容を確認してください。
