...
- ゲーム音声優先モード(ON時、VAD閾値を引き上げて小さい雑音より大きい音声を優先的に拾う)
...
- 発話が途切れずに続く長文では、無音区間の検出タイミングで文の途中が別のセグメントに分割されることがあり、文脈が失われて誤訳の原因になる場合がある
- 発話が非常に速く連続する場合、文字起こしの処理が追いつかず、古いセグメントが破棄されることがある(表示の遅延がどこまでも蓄積しないための仕様)
